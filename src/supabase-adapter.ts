...
export class SupabaseDatabaseAdapter extends DatabaseAdapter {
    ...
    async getMemoriesByRoomIds(params: {
        ...
        if (params.tableName) {
            query = query.eq("type", params.tableName);
        }
    ...
    > {
        const result = await this.supabase.rpc("get_embedding_list", {...opts, query_table_name: 'memories'});
        if (result.error) {
            throw new Error(JSON.stringify(result.error));
        }
        return result.data;
    ...
    async getMemories(params: {
        ...
        if (params.tableName) {
            query.eq("type", params.tableName)
        }
    ...
    async searchMemoriesByEmbedding(
        ...
        const queryParams = {
            query_table_name: "memories_1536",
            query_table_type: params.tableName,
            query_roomid: params.roomId,
    ...
    async createMemory(
        ...
        const createdAt = memory.createdAt ? new Date(memory.createdAt).toISOString() :new Date(Date.now()).toISOString();
        if (unique) {
            const opts = {
                // TODO: Add ID option, optionally
                query_id: uuid(),
                query_table_name: "memories_1536", // Memories table
                query_table_type: tableName, // memories.type column as existing table functionality
            ...
            const result = await this.supabase
                .from("memories_1536")
                .insert({ ...memory, id: uuid(), createdAt, type: tableName });
            const { error } = result;
            if (error) {
    ...
    async deleteCache(params) {
        try {
            const { error } = await this.supabase
                .from("cache")
                .delete()
                .match({ key: params.key, agentId: params.agentId });
